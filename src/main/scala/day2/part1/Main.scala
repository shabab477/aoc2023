...
object Main extends App {
  ...
  Using(Source.fromResource("day2_p1.txt")) { source =>
    ...
    val result = games.filter { game =>
      val validGameSets = game.gameSet.filter { gameSet =>
        gameSet.red <= RequiredRed && gameSet.green <= RequiredGreen && gameSet.blue <= RequiredBlue
      }

      validGameSets.length == game.gameSet.length
    }.map(_.id).sum
